%%
T = [1:length(datastolen)]/fs; % time in seconds
figure(4);
plot(T,datastolen)
xlabel('time [s]')
ylabel('signal [a.u.]')
...
for k =1:round(TMax/TWindow)
 ...
 clf
 hold all
 plot(T(1:NWindow),datastolen(1+NWindow*(k-1):NWindow+NWindow*(k-1)))
 box on
 title('Raw Signal')
 ylim([-0.1,0.1])
 xlabel('time [s]')
...
X = fft(datastolen(1+NWindow*(k-1):NWindow+NWindow*(k-1)),nfft);
X = fftshift(X);
